    ...
    for (const doc of sessionsSnapshot.docs) {
      ...
      if (data.nextSwagConfirm && now > data.nextSwagConfirm && !data.swagConfirmSent) {
        try {
          ...
          console.log(`✅ Sent swag confirm (${imageFilename}) to ${user}`);
        } catch (err) {
      ...
      if (data.nextFollowup3h && now > data.nextFollowup3h && !data.followup3hSent) {
        try {
          ...
          console.log(`✅ Sent 3-hour followup + updated stage to exchange for ${user}`);
        } catch (err) {
      ...
      if (data.nextFollowup23h && now > data.nextFollowup23h && !data.followup23hSent) {
        try {
          ...
          console.log(`✅ Sent 23-hour followup to ${user} (SID=${message.sid})`);
        } catch (err) {
